...
.DESCRIPTION
  ...
  CSV列 → hotels.yaml フィールドの対応：
    ...
    カテゴリ     → category（11→8種にマッピング。グッドネイバーはslug個別）
...
$areas = @(
  [ordered]@{ id="a"; tag="A"; name="舞浜エリア";   note="TDR直営・オフィシャル集中。パーク最至近。" }
...
# CSVカテゴリ → 表示用カテゴリ
$catMap = @{
  "ディズニーホテル（直営・ラグジュアリー）" = "直営（ラグジュアリー）"
  "ディズニーホテル（直営・バリュー）"       = "直営（バリュー）"
  "オフィシャルホテル"                       = "オフィシャルホテル"
  "オフィシャルホテル（一般ホテル区分）"     = "一般ホテル"
  "パートナーホテル"                         = "パートナーホテル"
  "一般ホテル"                               = "一般ホテル"
  "一般ホテル（温泉）"                       = "一般ホテル（温泉）"
  "一般リゾートホテル"                       = "一般リゾートホテル"
...
# 「グッドネイバーホテル」は施設ごとに表示区分が異なるため slug 個別指定
$catOverride = @{
  "dreamgate-maihama"   = "オフィシャルホテル"
  "henna-hotel-maihama" = "一般ホテル"
}

function Convert-Category($r) {
  if ($r.slug -and $catOverride.ContainsKey($r.slug)) { return $catOverride[$r.slug] }
  $c = $r."カテゴリ"
