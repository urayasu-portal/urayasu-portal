...
## 2026-08-07（第8回・travel-guide未確認分の一巡完了）

機械スキャン: 期日超過0件・日付直書き0件・facility-database.csv要対応0件。**内部リンク404を1件検出**（posts記事内、未執筆の別記事へのリンク）→ リンクを解除（機械的修正のため承認不要で対応）。

### Web照合・事実修正（2件）
| 記事 | 結果 |
|---|---|
| access-guide | 東京駅→舞浜駅の所要時間「約15分」が誤り。JR東日本公式メディア・東京ディズニーリゾート公式サイトともに「約20分」と明記されており訂正（2箇所）。舞浜駅→ディズニーランドの徒歩時間も記事「約10分」→公式「徒歩5分」に訂正。lastmod・factChecked更新 |
| taxi-guide | 空港定額運賃（舞浜リゾートキャブ・三ツ矢エミタスタクシー）、千葉県タクシー協会運賃（2026年3月16日改定後）すべて公式一致。修正なし・factChecked更新 |

### factChecked付与（7件）
| 記事 | 結果 |
|---|---|
| kids-emergency | 急病診療所の診療時間・住所・支払い方法・休日応急歯科診療所の診療時間すべて公式一致。修正なし |
| urayasu-map | 動的地図表示のみ、検証すべき事実記述なし。factChecked付与のみ |
| pre-trip-checklist（日本語版・レビュー用draft） | 英語版と同内容、公開版（英語）は前回確認済み |
| disney-tickets | 前回一次照合済みの内容を正式記録。修正なし |
| life-guide/_index | ハブページ（動的リスト表示）、検証すべき事実記述なし |

これで**travel-guideの主要日本語記事が一巡完了**（残るは多言語版のみ）。

### 持ち越し更新
- **factChecked未設定 残り**: 多言語版travel-guide群（en/zh/zh-tw/ko）。次回はこちらのローテーションへ
- 監視: 葛西臨海水族園の建て替え（新水族園2027年頃開業予定）
- 監視: urayasu-nyuyoji-shien の子どもインフル助成は季節事業（10月ごろFY2026ページ公開時に再リンク検討）

---

## 2026-08-08（第9回・ホテル比較系ハブ記事）

機械スキャン: 期日超過0件・日付直書き0件・facility-database.csv要対応0件・内部リンク404 0件。

前回の「多言語版travel-guide」ではなく、**未設定のままだったホテル比較系ハブ記事5本**（budget/compare/access/airport-limousine/coin-laundry、いずれもjaを含め全言語未設定）を優先して照合。compare/airport-limousine/coin-laundryはCSV駆動の動的生成ページ（layout: compare／facility-list）でFAQのみ静的、budget・accessは手書きの比較記事で具体的事実を多数含む。

### Web照合・事実修正（1件）
| 記事 | 結果 |
|---|---|
| hotels/budget（ja・en） | 浦安ビューフォートホテル「日本橋まで直通約16分」が誤り。東京メトロ東西線の公式時刻表実データでは18〜20分が目安（快速でも18分）のため「約18分」に訂正。その他の構造的事実（マイステイズ舞浜の徒歩6分・チェックアウト10:00、浦安サンホテルの朝食6:00・コインランドリー2F等）はすべて公式一致。ユーラシア舞浜アネックスの温泉特典は「割引」→実際は「いつでも半額」（記事の記載自体は誤りではない） |
| hotels/access（全5言語） | 羽田リムジンバス運賃1,300円一律・ハイアット停留所新設(2024年12月)・ディズニーアンバサダーホテル徒歩8分等は公式一致。浦安駅↔舞浜の路線バス運賃(240円)・所要時間・東西線の分数など一部項目は運営会社サイトの403エラーで一次情報に到達できず確認不能（推測で書き換えず据え置き） |

### factChecked付与（19ファイル・5記事×多言語）
budget(ja/en/zh/zh-tw/ko)・access(ja/en/zh/zh-tw/ko)・compare(ja/en/zh/zh-tw/ko)・airport-limousine(ja/en)・coin-laundry(ja/en)
...
## 2026-08-08（第10回・ハッピーエントリー記事＋多言語ローテーション）
...
| 記事 | 結果 |
|---|---|
| hotels/happy-entry（全5言語） | 「開園15分前まで入園」が東京ディズニーランドのみ正確で、**東京ディズニーシーは2025年9月からエントランス周辺の工事に伴い「開園5〜15分前（日により変動）」に運用変更**されていることが判明（複数の独立情報源で確認、公式一次情報は接続エラーのため二次情報で裏取り）。記事は両パーク一律「15分」と記載していたため、TDS部分を訂正。対象ホテルのリスト（ディズニーホテル6施設のみ）・オフィシャル/パートナー/独立系が対象外という区分・宿泊料金水準（2万円〜）は公式一致 |

### factChecked付与（20ファイル）
- hotels/happy-entry（ja/en/zh/zh-tw/ko）：上記の通り本文修正
- tokyo-sightseeing-access.zh／zh-tw、urayasu-traffic-park.zh：ja版で確認済みの数値と完全一致を確認のうえ付与
- urayasu-map（en/ko/zh-tw）：動的地図ページ、ja版と同様に検証すべき事実記述なし
- travel-guide/_index（ja/en/zh/zh-tw/ko）、hotels/_index.zh：ハブページ（タイトル・説明文のみ、動的リスト表示）

### 持ち越し更新
- **factChecked未設定 残り**: hotels配下の残り比較記事（kids/shuttle/types等、未調査）＋ travel-guide主要記事の残り多言語版
- 要再確認: hotels/access.mdの浦安駅↔舞浜路線バス運賃(240円)・所要時間（運営会社サイトの403エラーで未確認、継続持ち越し）
- 監視: 東京ディズニーシーのハッピーエントリー時間（5〜15分変動）は入口工事に伴う一時的措置。工事終了時期が判明次第、記事の表現を見直す
- 監視: 葛西臨海水族園の建て替え（新水族園2027年頃開業予定）
- 監視: urayasu-nyuyoji-shien の子どもインフル助成は季節事業（10月ごろFY2026ページ公開時に再リンク検討）

---

## 2026-08-08（第11回・hotels配下の残り比較記事7本）

機械スキャン: 期日超過0件・日付直書き0件・facility-database.csv要対応0件・内部リンク404 0件。

前回の持ち越しどおり、hotels配下で唯一残っていた7本（in-house-store/near-station/large-public-bath/pool/shuttle/types/kids）を消化。前者4本はCSV駆動の動的ページ（layout: facility-list）、後者3本（shuttle/types/kids）は手書きの内容豊富な記事で並列照合。

### Web照合・事実修正（2件）
| 記事 | 結果 |
|---|---|
| hotels/shuttle | ①ディズニーアンバサダーホテルが「ベイサイド・ステーション経由でモノレール乗換」と誤記載されていたが、実際は「ディズニーリゾートクルーザー」の別ルートでパークへ直行するため訂正（オークラ・グランドニッコー・ヒルトンの3軒のみがベイサイド経由）。②本文中「舞浜ユーラシア by HULIC」という誤った複合名（別ホテル「舞浜ユーラシア」と「舞浜ビューホテル by HULIC」の混同）を正しい表記に訂正。他の記述（パートナーホテルシャトル約15分、変なホテル舞浜の2026年7月減便等）はすべて公式一致 |
| hotels/types | ハッピーエントリーの説明が「開園15分前」で統一されていたが、前回(第10回)判明した東京ディズニーシーの工事に伴う変更（開園5〜15分前）を未反映だったため、FAQ・比較表・箇条書きの計4箇所を訂正。4区分（ディズニーホテル／オフィシャル／パートナー／グッドネイバー）の名称・対象ホテル・特典差は公式一致 |
| hotels/kids | 大浴場・部屋定員・シャトル・館内コンビニ等の設備情報はほぼ全て公式一致。ザ・ロイヤルパークホテル舞浜リゾート東京ベイの開業時期(2026年2月)・部屋定員も一致。修正なし |

### factChecked付与（7ファイル）
in-house-store・near-station・large-public-bath・pool（動的ページ、付与のみ）／shuttle・types（本文修正）／kids（修正なし）

これで**hotels配下の全ja記事のfactCheckedローテーションが一巡完了**。

### 持ち越し更新
- **factChecked未設定 残り**: travel-guide主要記事・hotels比較記事の多言語版（en/zh/zh-tw/ko）
- 要再確認: hotels/access.mdの浦安駅↔舞浜路線バス運賃(240円)・所要時間（継続持ち越し）
- 監視: 東京ディズニーシーのハッピーエントリー時間（5〜15分変動）は入口工事に伴う一時的措置。工事終了時期が判明次第、happy-entry.md・types.mdの表現を見直す
- 監視: 葛西臨海水族園の建て替え（新水族園2027年頃開業予定）
- 監視: urayasu-nyuyoji-shien の子どもインフル助成は季節事業（10月ごろFY2026ページ公開時に再リンク検討）
...
### life-guide-database.csv 第2弾（同日・107件→120件／収録範囲の拡大）

前段の「持ち越し」2件をユーザー判断で当日中に解消。あわせて**DBの収録範囲の定義を「記事掲載施設」から「記事掲載施設＋市公式一覧の公共施設」へ拡大**した（今後は記事に出てこない公共施設も収録してよい。記事未掲載の行は note に「記事未掲載(公式◯◯一覧より収録)」と明記して区別する）。

**ともづな7拠点を追加（type=government）**
| id | 名称 | 所在地 | 電話 | 担当地区 |
|---|---|---|---|---|
| tomozuna-nekozane | 猫実地域包括支援センター | 猫実1-1-1（市役所3階） | 047-381-9037 | 猫実一〜二丁目・堀江・富士見・港・鉄鋼通り・千鳥 |
| tomozuna-urayasu-ekimae | 浦安駅前地域包括支援センター | 北栄1-1-16 | 047-351-8950 | 当代島・北栄・猫実三〜五丁目 |
| tomozuna-shin-urayasu | 新浦安駅前地域包括支援センター | 入船1-2-1（プラザマーレ内） | 047-306-5171 | 入船・美浜・海楽 |
| tomozuna-takasu | 高洲地域包括支援センター | 高洲5-3-2（エスレ高洲1階） | 047-382-2424 | 高洲・明海・日の出 |
| tomozuna-tomioka | 富岡地域包括支援センター | 富岡3-1-9 | 047-721-1027 | 舞浜・東野・弁天・富岡・今川 |
| tomozuna-hinode-shisho | ともづな高洲 日の出支所 | 日の出2-15-2 | **047-383-9600** | ― |
| tomozuna-higashino-shisho | ともづな富岡 東野支所 | 東野3-4-11（ASMACI浦安1階） | 047-314-1085 | ― |

- 記事 urayasu-koreisha-kaigo.md の電話5件＋東野支所は公式と一致（訂正不要）。**日の出支所の電話047-383-9600は記事に未掲載＝今回新規に判明**
- type は既存語彙を維持するため government に収めた。高齢者福祉系が増えるなら welfare 型の新設を将来検討

**公園6件を追加（type=park／14→20件）**
| id | 名称 | 所在地 | 備考 |
|---|---|---|---|
| funawaku-ryokudo | 船圦緑道 | 当代島（精度:町丁目） | 緑道 |
| takasu-taiyo-no-oka-park | 高洲太陽の丘公園 | 高洲二丁目7番 | 半分が森林・テニスコート・駐20台 |
| hinode-ohisama-park | 日の出おひさま公園 | 日の出五丁目2番 | 約5,000平方mの芝生広場・修景池・噴水広場 |
| hinode-kita-park | 日の出北公園 | 日の出三丁目2番 | 海に面した球技広場・駐9台 |
| mihama-park | 美浜公園 | 美浜五丁目14番 | 健康遊具・新浦安駅徒歩8分 |
| sogo-park-daycamp | 総合公園デイキャンプ場 | 明海七丁目2番（総合公園内） | 全64区画・要予約 047-380-1295・9:00-16:30 |

座標は国土地理院ジオコーダで9件取得（HIT 9/9）。市役所3階・プラザマーレ・エスレ高洲・総合公園内の4件は同一建物の既存行から流用。
